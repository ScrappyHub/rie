...
function RIE-ParseJson([string]`$Path){
  if(-not (Test-Path -LiteralPath `$_Path -PathType Leaf)){ RIE-Die ("FILE_MISSING: " + `$_Path) }
}
"@
# ^ placeholder, replaced below
$newFn = $newFn.Replace('$_Path','$Path')
...
Write-Host ("PATCH_PARSEJSON_BODY_V4B_OK: " + $Lib) -ForegroundColor Green
