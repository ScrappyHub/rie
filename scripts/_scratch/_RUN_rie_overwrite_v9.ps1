...
function Run-Child([string]$ScriptPath,[hashtable]$ArgMap,[string]$OutLog,[string]$ErrLog){
  ...
  foreach($k in @(@($ArgMap.Keys))){ [void]$args.Add("-" + [string]$k); $v=$ArgMap[$k]; if($null -eq $v){ $v="" }; [void]$args.Add([string]$v) }
  EnsureDir (Split-Path -Parent $OutLog); EnsureDir (Split-Path -Parent $ErrLog)
...
Write-Host "RIE_OVERWRITE_V9_START" -ForegroundColor Cyan
...
Write-Host "RIE_OVERWRITE_V9_OK" -ForegroundColor Green
