...
Write-Host "RIE_OVERWRITE_V4_START" -ForegroundColor Cyan

# 1) Write schema files using SAFE single-quoted here-strings (no escaping)
$schemaSourceRecord = @'
{
...
# 2) Minimal lib + selftest (you can expand later; this gets us GREEN + stops console-loss)
...
# Positive vector (inline) written as deterministic file then validated
...
$src = @'
{
  ...
  "provenance": {
    ...
    "retrieved_at_utc": "2026-02-23T00:00:00Z"
  },
  "tags": ["demo"]
}
'@
...
# Negative vector: forbidden prop should fail
$bad = @'
{
  "schema": "rie.source_record.v1",
  "source_id": "s_bad_001",
...
Write-Host "RIE_OVERWRITE_V4_OK" -ForegroundColor Green
