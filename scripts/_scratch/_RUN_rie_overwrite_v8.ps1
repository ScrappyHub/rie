...
function Run-Child([string]$ScriptPath,[hashtable]$ArgMap,[string]$OutLog,[string]$ErrLog){
  ...
  EnsureDir (Split-Path -Parent $OutLog)
  EnsureDir (Split-Path -Parent $ErrLog)
...
Write-Host "RIE_OVERWRITE_V8_START" -ForegroundColor Cyan

# --- schemas (use here-strings in the runner; these are safe because we are writing the runner line-by-line)
$schemaSourceRecord = @'
{
...
function RIE-AssertHasProps($o,[string[]]$names,[string]$ctx){
  foreach($n in @(@($names))){
    if(-not (RIE-HasProp $o $n)){ RIE-Die ("MISSING_PROP: " + $ctx + " :: " + $n) }
  }
}

function RIE-AssertNoProps($o,[string[]]$names,[string]$ctx){
  foreach($n in @(@($names))){
    if(RIE-HasProp $o $n){ RIE-Die ("FORBIDDEN_PROP: " + $ctx + " :: " + $n) }
  }
}
...
try {
  RIE-ValidateSourceRecordV1 $b "tv.bad"
...
# run selftest in child with logs
...
Write-Host "RIE_OVERWRITE_V8_OK" -ForegroundColor Green
